...
def compute_cycle_spd(
    ...
    window_offset = (
        pd.DateOffset(days=window_days) if window_days is not None else WINDOW_OFFSET
    )
...
def check_strategy_submission_ready(
    ...
    start = start_date or BACKTEST_START
    backtest_end = end_date or BACKTEST_END or dataframe.index.max().strftime("%Y-%m-%d")
    backtest_df = dataframe.loc[start:backtest_end]
